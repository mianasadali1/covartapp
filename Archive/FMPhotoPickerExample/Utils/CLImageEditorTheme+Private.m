...
+ (NSString*)localizedString:(NSString*)key withDefault:defaultValue
{
    ...
    NSLog(@"%@",[CLImageEditorTheme bundle]);
...
{
    return [[SubToolBarItem alloc] initWithFrame:frame target:target action:action toolInfo:toolInfo isIcon:isIcon isPro:isPro];
}
...
+ (StickerToolBarItem*)stickerMenuItemWithFrame:(CGRect)frame target:(id)target action:(SEL)action toolInfo:(ToolInfo*)toolInfo isIcon:(BOOL)isIcon isPro:(BOOL)isPro{
    return [[StickerToolBarItem alloc] initWithFrame:frame target:target action:action toolInfo:toolInfo isIcon:isIcon isPro:isPro];
