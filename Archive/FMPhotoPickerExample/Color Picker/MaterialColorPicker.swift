...
//  }
  ...
  @objc dynamic open var cellSpacing: CGFloat = 2
    ...
    @objc dynamic open var callingClass: String?
  ...
  lazy var collectionView: UICollectionView = {
    ...
    collectionView.register(UICollectionViewCell.self, forCellWithReuseIdentifier: "ColourCell")
  ...
  //Select index programtically
    ...
    public func numberOfSections(in collectionView: UICollectionView) -> Int {
        return self.callingClass == "color" ? 2 : 1
    }
  //MARK: CollectionView DataSouce
  open func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
    return self.callingClass == "color" ? ( section == 0 ? 1 : colors.count ) : colors.count
  }
  
  open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    if indexPath.section == 0 {
        if self.callingClass == "color" {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "ColourCell", for: indexPath)
            let imageview = UIImageView(image: UIImage(named: "icColorPalette"))
  ...
  open func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    if indexPath.section == 0 {
        if self.callingClass == "color" {
            self.delegate?.didSelectColorAtIndex(self, index: 1000, color: colors[indexPath.row])
        } else {
            selectedIndex = indexPath
            animateCell()
        }
