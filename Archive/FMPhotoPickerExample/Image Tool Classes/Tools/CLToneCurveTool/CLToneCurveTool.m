...
- (void)setup
{
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
        _menuContainer.frame        =   CGRectMake(100, self.editor.view.height-550, self.editor.view.width - 200, 550);
        _menuContainer.center       =   self.editor.view.center;
    }
    
   // _menuContainer.backgroundColor  =    [UIColor colorWithWhite:0 alpha:0.2];
    
    [self.editor.view addSubview:_menuContainer];
    
    
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
        _tonecurveView              =   [[CLToneCurveView alloc] initWithSuperview:_menuContainer frame:CGRectMake(0, 25, _menuContainer.width, 500)];

    }
    else{
        _tonecurveView              =   [[CLToneCurveView alloc] initWithSuperview:_menuContainer frame:CGRectMake(0, 20, _menuContainer.width, 240)];
    }
    _tonecurveView.delegate         =   self;
    _tonecurveView.backgroundColor  =   [UIColor colorWithWhite:0 alpha:0.2];
    _tonecurveView.gridColor        =   [UIColor whiteColor];
    _tonecurveView.pointColor       =   [UIColor whiteColor];
    _tonecurveView.lineColor        =   [UIColor whiteColor];
    ...
    [UIView animateWithDuration:kCLImageToolAnimationDuration
                     animations:^{
                         if(CGAffineTransformIsIdentity(_menuContainer.transform)){
                             _menuContainer.transform = CGAffineTransformMakeTranslation(0, self.editor.view.height-_menuContainer.top-self.editor.menuScrollView.height);
