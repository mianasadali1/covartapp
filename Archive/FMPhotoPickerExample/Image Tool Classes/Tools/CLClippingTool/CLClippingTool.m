...
@interface CLRatioMenuItem : ToolBarItem
...
+ (NSArray*)defaultPresetRatios
{
    return @[
             ...
             @{kCLClippingToolRatioValue1:@5, kCLClippingToolRatioValue2:@7, kCLClippingToolRatioTitleFormat:@"%g : %g"},
             @{kCLClippingToolRatioValue1:@8, kCLClippingToolRatioValue2:@10, kCLClippingToolRatioTitleFormat:@"%g : %g"},
...
- (void)setup
{
    ...
    _menuContainer = [[UIView alloc] initWithFrame:self.editor.menuScrollView.frame];
    _menuContainer.backgroundColor = self.editor.menuScrollView.backgroundColor;
...
//    
    ...
    _gridView.bgColor       =   [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];

    _gridView.gridColor     =   [UIColor lightGrayColor];
...
- (void)setCropMenu
{
    ...
    for(NSDictionary *info in ratios){
        ...
        CLRatioMenuItem *view = [[CLRatioMenuItem alloc] initWithFrame:CGRectMake(x, 0, _menuScroll.height, _menuScroll.height) target:self action:@selector(tappedMenu:) toolInfo:nil isIcon:true];
        view.iconView.image = [UIImage imageNamed:@"ratio.png"];
        view.ratio = ratio;
        view.iconView.layer.borderColor  =   [UIColor darkGrayColor].CGColor;
        view.iconView.layer.borderWidth  =   1.0;
        view.iconView.frame = CGRectMake(10, 10, 60, 60);
...
- (void)refreshViews
{
    ...
    UILabel *ratioLbl   =   [[UILabel alloc]initWithFrame:CGRectMake(10, 10, 60, 60)];
    ratioLbl.text       =   [_ratio description];
    ratioLbl.font       =   [UIFont systemFontOfSize:12];
    ratioLbl.textColor  =   [UIColor darkGrayColor];
    ratioLbl.textAlignment  = NSTextAlignmentCenter;
    ratioLbl.backgroundColor    =   [UIColor clearColor];
    [self addSubview:ratioLbl];
...
//    }
//    else{
//        CGFloat maxW  = MAX(self.iconView.image.size.width, self.iconView.image.size.height);
//        W = 50 * self.iconView.image.size.width / maxW;
//        H = 50 * self.iconView.image.size.height / maxW;
//    }
//    self.iconView.frame = CGRectMake(center.x-W/2, center.y-H/2, W, H);
}
